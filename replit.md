...
- **KHURK OS app dock**: Slide-out app dock with macOS-style icons; `AppWindow.tsx` renders apps as overlaid panels. Ballpoint Notes and Foldr run as native React panels (no iframe). All other KHURK apps continue to render in iframes.
  ...
  - **FoldrPanel v2** (`src/components/khurk/apps/FoldrPanel.tsx`): Full-featured macOS Finder-style cloud file manager backed by Cloudflare R2 object storage. Features: 6 themes (Midnight/Slate/Forest/Ocean/Sunset/Snow), folder hierarchy with sidebar tree + breadcrumb navigation, drag/drop sorting, list + grid view toggle, right-click context menus, details drawer (preview + download + star + rename + trash), inline folder creation, Trash section with restore + permanent delete, 100MB upload limit.
  - **Foldr encryption**: All files AES-256-GCM encrypted server-side before upload to R2. Per-file random 32-byte key; key encrypted with master `FOLDR_ENCRYPTION_KEY` and stored in DB. `cid` column repurposed to store R2 object key. Download via `/api/foldr/files/:id/content` proxy which fetches from R2 and decrypts on-the-fly. Encrypted with `artifacts/api-server/src/lib/foldr-crypto.ts`.
  - **DB tables**: `foldr_folders` (id, userId, name, parentId, sortOrder, createdAt) + `foldr_files` (id, userId, folderId, name, size, mimeType, cid [stores R2 key], isEncrypted, encryptedKey, isStarred, sortOrder, uploadedAt, deletedAt) + `ballpoint_notes` (id, userId, title, content as HTML, isPinned, isArchived, isTrashed, timestamps)
  - **Foldr API routes**: `GET/POST /api/foldr/folders`, `PATCH/DELETE /api/foldr/folders/:id`; `POST /api/foldr/upload` (encrypts then stores to R2), `GET /api/foldr/files` (filter by folderId/starred/trash), `GET /api/foldr/files/:id/content` (R2 fetch + decrypt proxy), `PATCH /api/foldr/files/:id`, `DELETE /api/foldr/files/:id` (soft/hard — hard also deletes R2 object)
