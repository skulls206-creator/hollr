...
import { getListDmMessagesQueryKey } from '@workspace/api-client-react';
...
async function toggleDmReaction(threadId: string, messageId: string, emojiId: string) {
  const res = await fetch(
    `/api/dms/${threadId}/messages/${messageId}/reactions/${encodeURIComponent(emojiId)}`,
    { method: 'PUT' }
  );
...
export function DmReactionPills({
  ...
  const { mutate: toggle } = useMutation({
    mutationFn: (emojiId: string) => toggleDmReaction(threadId, messageId, emojiId),
    onSuccess: (updated) => {
    ...
    <div className="flex flex-wrap gap-1 mt-1 items-center relative">
      ...
      {showAddButton && (
        <div className="relative">
          ...
          {pickerOpen && (
            <EmojiPickerPopover
              onEmojiClick={(emoji) => { toggle(emoji); setPickerOpen(false); }}
