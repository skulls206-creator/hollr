...
- **KHURK OS app dock**: Slide-out app dock with macOS-style icons; `AppWindow.tsx` renders apps as overlaid panels. Ballpoint Notes and Foldr run as native React panels (no iframe). All other KHURK apps continue to render in iframes.
  ...
  - **FoldrPanel v2** (`src/components/khurk/apps/FoldrPanel.tsx`): Full-featured macOS Finder-style cloud file manager (IPFS via Lighthouse). Features: 6 themes (Midnight/Slate/Forest/Ocean/Sunset/Snow), folder hierarchy with sidebar tree + breadcrumb navigation, drag/drop sorting, list + grid view toggle, right-click context menus, details drawer (preview + IPFS CID + download + star + rename + trash), inline folder creation, Trash section with restore + permanent delete, 100MB upload limit.
  - **Foldr encryption**: All files AES-256-GCM encrypted before upload to Lighthouse. Per-file random 32-byte key; key encrypted with master `FOLDR_ENCRYPTION_KEY` and stored in DB. Download via `/api/foldr/files/:id/content` proxy which decrypts on-the-fly. Encrypted with `artifacts/api-server/src/lib/foldr-crypto.ts`.
  - **DB tables**: `foldr_folders` (id, userId, name, parentId, sortOrder, createdAt) + `foldr_files` (id, userId, folderId, name, size, mimeType, cid, isEncrypted, encryptedKey, isStarred, sortOrder, uploadedAt, deletedAt) + `ballpoint_notes` (id, userId, title, content as HTML, isPinned, isArchived, isTrashed, timestamps)
  - **Foldr API routes**: `GET/POST /api/foldr/folders`, `PATCH/DELETE /api/foldr/folders/:id`; `POST /api/foldr/upload` (encrypts before IPFS), `GET /api/foldr/files` (filter by folderId/starred/trash), `GET /api/foldr/files/:id/content` (decrypt proxy), `PATCH /api/foldr/files/:id`, `DELETE /api/foldr/files/:id` (soft/hard)
